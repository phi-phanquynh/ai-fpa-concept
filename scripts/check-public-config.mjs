...
const scanTargets = ["index.html", path.join("dist", "index.html")].filter((file) => fs.existsSync(file));
const requiredFiles = [
  workflowPath,
...
const requiredPatterns = [
  { label: "canonical", pattern: new RegExp(`<link\\s+rel=["']canonical["']\\s+href=["']${siteUrl}["']\\s*/?>`, "i") },
  ...
  { label: "og:url", pattern: new RegExp(`<meta\\s+property=["']og:url["']\\s+content=["']${siteUrl}["']\\s*/?>`, "i") },
  {
    label: "og:image",
    pattern: new RegExp(`<meta\\s+property=["']og:image["']\\s+content=["']${siteUrl}og-image\\.svg["']\\s*/?>`, "i"),
  },
...
for (const file of scanTargets) {
  ...
  if (file.endsWith(path.join("dist", "index.html")) && !html.includes(`${basePath}assets/`)) {
